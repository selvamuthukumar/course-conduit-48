...
-- Update courses policies for role-based access
DROP POLICY IF EXISTS "Anyone can create courses" ON public.courses;
CREATE POLICY "Admins and managers can create courses"
  ON public.courses
  FOR INSERT
  WITH CHECK (
...
CREATE POLICY "Admins and managers can update courses"
  ON public.courses
  FOR UPDATE
  USING (
    public.has_role(auth.uid(), 'admin') OR 
...
-- Update enrollments policies for authenticated users
